...
export function assertSafeIdentifier(value: string, label: string): void {
  ...
  if (value.length > MAX_IDENTIFIER_LENGTH) {
    throw new Error(
      `${label}: exceeds maximum length of ${MAX_IDENTIFIER_LENGTH} characters`,
    );
  }
  if (!SAFE_IDENTIFIER_RE.test(value)) {
    throw new Error(
      `${label}: contains unsafe characters (allowed: A-Z a-z 0-9 _ - / .)`,
    );
  }
...
export function validateCommand(command: string): CommandValidationResult {
  ...
  const matchedPrefix = ALLOWED_PREFIXES.find(
    (prefix) =>
      command === prefix ||
 ...
 */
export const ALLOWED_COMMAND_PREFIXES: readonly string[] = ALLOWED_PREFIXES;

/**
 * Read-only view of the denylist (exported for test/diagnostic use).
 */
export const FORBIDDEN_COMMAND_TOKENS: readonly string[] = FORBIDDEN_SUBSTRINGS;
