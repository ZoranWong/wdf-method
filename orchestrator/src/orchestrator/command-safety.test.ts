...
describe('assertSafeIdentifier', () => {
  ...
  describe('rejects empty or invalid input', () => {
    ...
    it('rejects undefined cast as string', () => {
      expect(() =>
        assertSafeIdentifier(undefined as unknown as string, 'branch'),
      ).toThrow(/string/);
    });
    it('rejects null cast as string', () => {
      expect(() =>
        assertSafeIdentifier(null as unknown as string, 'branch'),
      ).toThrow(/string/);
    });
    it('rejects number cast as string', () => {
      expect(() =>
        assertSafeIdentifier(123 as unknown as string, 'branch'),
      ).toThrow(/string/);
    });
  });
  ...
  describe('length bounds', () => {
    ...
    it('rejects over the max length', () => {
      ...
      expect(() => assertSafeIdentifier(value, 'branch')).toThrow(
        /maximum length/,
      );
    });
  });
...
describe('validateCommand', () => {
  ...
  describe('rejects empty or invalid input', () => {
    ...
    it('rejects undefined cast as string', () => {
      const r = validateCommand(undefined as unknown as string);
      expect(r.ok).toBe(false);
      expect(r.reason).toMatch(/string/);
    });
    it('rejects null cast as string', () => {
      const r = validateCommand(null as unknown as string);
      expect(r.ok).toBe(false);
      expect(r.reason).toMatch(/string/);
    });
  });
