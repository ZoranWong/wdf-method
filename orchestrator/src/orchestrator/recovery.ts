 ...
 */
function tryRestoreFromBackup(filename: string, statusDir: string, backupDir: string): boolean {
 ...
 */
function rebuildFromSplitFiles(statusDir: string): string {
  const lines: string[] = [];
 ...
 */
function makeDashboard(result: Omit<RecoveryResult, 'dashboard'> & { rebuiltDerivedStatus: boolean }): RecoveryResult {
