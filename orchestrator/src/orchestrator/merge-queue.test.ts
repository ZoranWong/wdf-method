...
describe('merge queue safety', () => {
  it('rejects unsafe branch names', () => {
    expect(() => validateMergeQueueItem({ branch: 'story/S-1;rm -rf /', story_id: 'S-1', queue_item_id: 'QUEUE-1', integration_checks: ['npm run test'] } as any)).toThrow(/branch/);
  });
...
describe('hidden overlap detection', () => {
  ...
  it('detects multiple hidden overlaps', () => {
    expect(detectHiddenOverlapsFromFileLists(
      ['src/a.ts', 'shared/config.ts', 'shared/logger.ts'],
      ['src/b.ts', 'shared/config.ts', 'shared/logger.ts'],
      ['src/a.ts'],
      ['src/b.ts']
    )).toEqual(['shared/config.ts', 'shared/logger.ts']);
  });
});
