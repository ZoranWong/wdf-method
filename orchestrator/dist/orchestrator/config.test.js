...
import { parseToml, loadConfig, resolvePath, getOutputDir, getSprintTrackingPath, getStatusDir, getStoriesDir, getMergeQueueDir, getSignalDir, getAuditDir, DEFAULT_CONFIG, } from './config.js';
let tmpRoot;
beforeEach(() => {
...
describe('path helpers', () => {
    ...
    it('getSprintTrackingPath resolves to sprint-status.yaml', () => {
        const { config } = loadConfig(tmpRoot, { silent: true });
        expect(getSprintTrackingPath(config, tmpRoot)).toBe(resolve(tmpRoot, '_bmad-output/web-dev-flow/sprint-status.yaml'));
    });
    ...
    it('getMergeQueueDir falls back to status/merge-queue', () => {
        const { config } = loadConfig(tmpRoot, { silent: true });
        expect(getMergeQueueDir(config, tmpRoot)).toBe(resolve(tmpRoot, '_bmad-output/web-dev-flow/status/merge-queue'));
    });
...
`);
    ...
    it('getAuditDir is sibling to sprint-status.yaml', () => {
        const { config } = loadConfig(tmpRoot, { silent: true });
        expect(getAuditDir(config, tmpRoot)).toBe(resolve(tmpRoot, '_bmad-output/web-dev-flow/audit'));
    });
