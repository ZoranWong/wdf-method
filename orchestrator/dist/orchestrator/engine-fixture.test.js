...
describe('engine fixture critical path', () => {
    it('fails closed, validates SRG, backs up state, and recovers derived status', async () => {
        ...
        const gateResult = await gate.evaluate({
            phase: 1,
            checks: [
                { id: 'X', type: 'field_exists', field: 'missing.value', description: 'missing' },
            ],
            all_pass: false,
        }, state);
        ...
        expect(recovery.actions.some((a) => /delete|reset|revert|clean|remove/i.test(a))).toBe(false);
    });
    ...
    it('gate evaluator rejects unknown check types', async () => {
        ...
        mkdirSync(statusDir, { recursive: true });
        writeFileSync(join(statusDir, 'global.yaml'), 'global_state:\n  project: test\n  workflow_version: 3.7.0\n');
        const state = await SprintStatusManager.loadFromStatusDir(statusDir, join(root, '_bmad-output', 'wdf-method', 'sprint-status.yaml'));
        const gate = new GateEvaluator(root);
        const result = await gate.evaluate({
            phase: 1,
            checks: [{ id: 'T1', type: 'unknown_made_up_type', description: 'test' }],
            all_pass: false,
        }, state);
...
//# sourceMappingURL=engine-fixture.test.js.map